...
\centerline{April 16, 2026}

\

\centerline{Cliff Joslyn, Ph.D., Advisor}
\centerline{Visiting Professor, Systems Science and Industrial Engineering, Binghamton University}

\

\centerline{Carlos Gershenson-Garcia, Ph.D., Committee Chair}
\centerline{Professor of Empire Innovation, Systems Science and Industrial Engineering, Binghamton University}

\

\centerline{Sadamori Kojaku, Ph.D., Committee Member}
\centerline{Assistant Professor, Systems Science and Industrial Engineering, Binghamton University}
}
...
[Acknowledgments placeholder --- thank advisor, committee, family, funding sources, etc.]
\end{doublespace}
...
Our analysis addresses all three gaps. We validate centrality measure robustness through systematic filtration and boundary sensitivity analysis, revealing that all measures remain stable under filtration when edge weights represent trade volume rather than distance, while betweenness shows high sensitivity to boundary specification. We then use this validated framework to comprehensively measure state economic power across macro, meso, and micro levels using Jang \& Yang's three-level centrality approach \cite{jang_network_2020, jang_measuring_2023}. Finally, we show how specifying the existence of an international trade boundary through the addition of a 52nd ``Rest of World'' node significantly influences network structure.
...
Both datasets draw from mandatory reporting requirements and Census Bureau validation procedures, providing a reasonable degree of confidence that data quality meets a standard necessary for rigorous network analysis.
...
\caption{Three-level centrality framework adapted from Jang \& Yang (2023). Highlighted nodes indicate centrality levels: red (high betweenness), green (high eigenvector), gray (low/baseline). Node size reflects relative centrality score.}
...
\begin{enumerate}
    ...
    \item \textbf{Top-K Overlap.} Jaccard similarity \cite{jaccard1912} measures whether the same states occupy leadership positions in both networks. For each threshold $k$ (top-5, top-10, top-20), $J_k = |S_{51} \cap S_{52}| / |S_{51} \cup S_{52}|$, where $S_{51}$ and $S_{52}$ are the top-$k$ states in each network. Values below 0.6 indicate substantial leadership turnover due to boundary specification.
...
This analysis only examines state economic centrality through commodity trade flows. As a result, there are several constraints on interpretation and generalizability that must be acknowledged.
...
Eigenvector centrality correlates strongly with GDP ($\rho = 0.934$), as does out-degree ($\rho = 0.904$). Betweenness diverges: at $\rho = 0.763$, it captures something GDP does not. Population produces nearly identical rankings (GDP--population $r = 0.98$), so we use GDP as our baseline throughout. All six centrality--control scatter plots are in Appendix~\ref{appendix:scatters}.

Table~\ref{tab:top10} presents raw centrality rankings for reference.

\begin{table}[ht]
    \centering
    \caption{Top 10 States by Centrality Measure (51 $\times$ 51 Domestic Network).
...
\FloatBarrier
Betweenness centrality shows the starkest divergence. New Jersey ranks 8th in GDP (\$602B) with zero betweenness. Like Florida, it lies on no shortest paths. In total, 31 states (61\%) have zero betweenness, including 5 of the top 25 economies. Trade routes concentrate through a handful of hubs --- only 20 states (39\%) have nonzero betweenness, led by CA, TX, NY, PA, and IL.
...
\caption{Geographic distribution of GDP-centrality divergence. Green: states ranking higher in centrality than GDP predicts (e.g., KY +14, IN +6, TN +6). Purple: states ranking lower (e.g., WA -8, CO -9, MA -11).}
\label{fig:physical_economy}
\end{figure}

Table~\ref{tab:divergence} summarizes the extreme cases. The eight largest overperformers include six states east of the Mississippi; the five most significant underperformers include three of the four highest-GDP coastal states. 
...
($\rho = 0.994$): the top-5 producers (CA, TX, IL, PA, OH) hold position regardless of boundary specification, because raw output volume doesn't depend on network routing. The more a centrality measure depends on global network structure, the more boundary specification seems to matter. 
...
\caption{Betweenness rank changes between domestic and international networks. Points on diagonal indicate no rank change. Green: improved; red: declined.}
...
\caption{Geographic distribution of eigenvector rank changes when international flows are added. Green: improved ranking; purple: declined.}
...
Industry composition data from the Bureau of Economic Analysis \cite{bea_gdp_state_2024} supports both patterns. Overperforming states have manufacturing as their largest or second-largest sector, while underperforming states are dominated by finance, professional services, and technology.
...
Segarra and Ribeiro \cite{segarra_stability_2015} proved that betweenness centrality is mathematically discontinuous under standard computation, leading some researchers to avoid it in dense networks. Our filtration analysis shows this concern does not apply when proper weight inversion is used. All three centrality measures show perfect rank stability ($\rho = 1.000$) under 33\% edge removal. In this network, betweenness instability appears to be an artifact of incorrect weight treatment, not the inherent mathematical limitation identified by Segarra and Ribeiro. With correct weight inversion, betweenness reliably identifies bridging positions even at 99.4\% network density.
...
\centering
\includegraphics[width=\textwidth]{figures/gdp_vs_eigenvector_scatter.png}
\caption{GDP rank vs.\ eigenvector centrality rank ($\rho =
...
\caption{GDP rank vs.\ population rank (Spearman $\rho = 0.975$). The near-perfect correlation confirms that GDP and population produce consistent control rankings, validating the use of either as a baseline for centrality divergence analysis.}                 
...
\caption{Pop.\ in millions (2017 ACS). GDP in billions (2017 Q4 BEA). Betw.\ = betweenness centrality (normalized). Eig.\ = eigenvector centrality (normalized). Out-D.\ = weighted out-degree centrality (normalized). Rk = rank (1 = highest). States sorted by eigenvector centrality rank.} \label{tab:node_distribution} \\
...
\caption{Eigenvector centrality rank stability ($\rho = 0.982$, 84\% changed). Gateway states (CA, TX, NY) show
...
\caption{Weighted out-degree rank stability ($\rho = 0.994$, 55\% changed). Highest stability among all
