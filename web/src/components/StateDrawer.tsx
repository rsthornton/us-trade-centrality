...
function RankBadge({ rank, total = 51 }: RankBadgeProps) {
  const pct = rank / total;
  const color =
    pct <= 0.2 ? "var(--accent-green)" : pct <= 0.5 ? "var(--accent-blue)" : "var(--text-muted)";
  return (
    <span
...
function formatDollars(raw: number): string {
  const b = raw / 1e9;
    ...
    <div className={inline ? "p-4" : "p-5"}>
      ...
      <div className="space-y-3">
        ...
        {MEASURES.map(({ key, label }) => (
          <div key={key} className="p-3 rounded-lg" style={{ backgroundColor: "var(--bg-surface)" }}>
            <div className="text-xs uppercase tracking-wider mb-1" style={{ color: "var(--text-muted)" }}>
              {label}
            </div>
            <div className="flex justify-between items-center">
            ...
            {topOutbound.map((p, i) => (
              ...
              >
                <div className="flex items-center gap-2 min-w-0 flex-1 mr-2">
                  <span className="text-xs w-4 text-right flex-shrink-0" style={{ color: "var(--text-muted)" }}>
                    {i + 1}
                  </span>
                ...
                <span className="font-mono text-sm flex-shrink-0" style={{ color: "var(--text-secondary)" }}>
